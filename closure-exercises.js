...
function once(func) {
	let output
  function checkOnce(y){
    output = output || func(y);
    return output;
  }
  return checkOnce
}
...
function rollCall(names) {
	let index = 0;
  function fn (){
   	if (index === names.length) return console.log("Everyone accounted for")
    else {
      index++
      return console.log(names[index-1])
    }
  }
  return fn;
}
...
function censor() {
  let data = {};
  function fn(firstString, secondString){
    let output = firstString;
    if(secondString === undefined){
      for (let word in data) {
      	if (firstString.indexOf(word) > 0) {
          output = output.replace(word, data[word])
        }
      }
      return output;
    }
...
function average() {
  let nums = [];
  function fn(arg) {
    if(arg) nums.push(arg);
    let sum = nums.reduce(( acumulador, valorAtual ) => acumulador + valorAtual,0);
...
function makeFuncTester(arrOfTests) {
  function fn(cb) {
    for (let i = 0 ; i < arrOfTests.length; i++) {
      if (!arrOfTests[i][1].includes(cb(arrOfTests[i][0]))) return false;
    }
  return true
  }
  return fn;
...
// console.log(myActions('undo')); // => should log 'nothing to undo' // correction: should be walk undone;
